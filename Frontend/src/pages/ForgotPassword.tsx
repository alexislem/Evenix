...
const ForgotPassword: React.FC = () => {
  ...
  const handleCheckEmail = async (e: React.FormEvent) => {
    ...
    try {
      // Appel au backend pour récupérer la question (Endpoint à créer)
      // GET /api/auth/security-question?email=...
      const response = await api.get(`/api/auth/security-question?email=${email}`);
      setSecurityQuestion(response.data.question); // Le backend doit renvoyer { question: "..." }
      setStep(2);
    } catch (err: any) {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      // Appel au backend pour reset (Endpoint à créer)
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4 py-12">
      <div className="max-w-md w-full bg-gray-800 rounded-2xl shadow-2xl p-8 border border-gray-700">
        ...
        {error && (
          <div className="bg-red-900/30 border border-red-700 text-red-200 px-4 py-3 rounded-lg mb-6 flex items-center text-sm">
